setwd("~/Desktop/cognitive-spike-2016")
PATHTOSPIKECSV = 'CSV/'
...
packages= c("numDeriv","RCurl","ggplot2","reshape2","plyr","lme4","nlme","car","lmtest","coefplot2","GGally","scales",'Rmisc',"tidyr","effsize","boot",'grid')
lapply(packages,usePackage)

#CHECK CONVERGENCE OF MIXTURE MODELS
...
##### SUPP FIG ##### 
# WORD RANDOMIZATION
tmp = ddply(recallDat,.(word,word_order),summarize,recallMean=mean(as.numeric(recalled)-1))
tmp = tmp[tmp$word_order>5,]
ggplot(dat=tmp,aes(x=word,y=recallMean)) + geom_bar(stat='identity')
ggplot(dat=tmp,aes(x=as.factor(word_order),y=recallMean)) + geom_bar(stat='identity')
...
#determine number of electrodes in left and right cerebrum for normalization
rlch = read.csv('rlch.csv')
rlch = as.data.frame(rlch)
tmpavg2 = cbind(tmpavg2,rlch$NumCh)
colnames(tmpavg2)[5] = 'NumCh'
...
#test if detected spikes lateralize as expected
...
mod = lm(dat=tmp,recallMean~word_order)
...
#sum number missed for each word for each patient
tmp = recallDat[recallDat$spikes_ja_spatial>=1,]
a = ddply(tmp, .(patient,word_order,onset,recalled),summarize,avgSpikes = mean(spikes_ja_spatial),
...
groupmod = glmer(recalled ~ spikes_ja_spatial + word_order + age + (1|patient/session), data = onlysz, family=binomial,control=glmerControl(optimizer="bobyqa"),nAGQ=1)
checkConv(groupmod)
...
# combine SZ/nonsz onset columns
data_long = gather(onlysz,szonset,spikes,SzOnset_spikes:non_SzOnset_spikes)
ggplot(dat=data_long,aes(x=szonset,y=spikes)) + geom_jitter() + facet_wrap(~onset)
#Do patients with left versus right sided epilepsy have different regional effects?
...
a = a[order(a$P),]
regionsAll = a
write.csv(regionsAll,file='regionsorig.csv')
...
tmpmod = glmer(data=recallDat,recalled~word_order + age + L_Brodmannarea37 + L_Brodmannarea20 + L_Brodmannarea8 + (1|patient/session),family=binomial,control=glmerControl(optimizer="bobyqa"),nAGQ=1)
...
ggplot(dat=c,aes(x=variable,y=value,colour=onset)) + geom_boxplot() + geom_hline(yintercept=0) + xlab('Region') +
  ...
  theme(axis.text=element_text(size=12),axis.title=element_text(size=16,face="bold"),  plot.title = element_text(size=16, face="bold", margin = margin(10, 0, 10, 0),vjust=1, lineheight=0.6), legend.text=element_text(size=12),legend.title=element_text(size=12),legend.position='right') 
